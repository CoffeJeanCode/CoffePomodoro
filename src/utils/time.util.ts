...
export const getCurrentWeek = (currentDate: Date) => {
	...
	return Math.ceil(
		Math.floor(
			($date.valueOf() - startYearDate.valueOf()) / (24 * 60 * 60 * 1000),
		) / 7,
	);
};
...
export const getStartOfWeek = (date: Date, weekStartsOn: number): Date => {
	const day = date.getDay();
	const diff = (day < weekStartsOn ? 7 : 0) + day - weekStartsOn;
	return new Date(date.getFullYear(), date.getMonth(), date.getDate() - diff);
};

export const getEndOfWeek = (date: Date, weekStartsOn: number): Date => {
	const day = date.getDay();
