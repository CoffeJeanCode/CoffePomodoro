...
const SchemaSettingItem: FC<SchemaSettingItemProps> = ({ schema }) => {
	...
	const { currentSchemaId, updateSchema, deleteSchema, setCurrentSchema } = useSchemasState();
	...
	return (
		<Chip value={schema.id} onClick={() => setCurrentSchema(currentSchemaId === id ? "" : currentSchemaId)}>
			<Flex
