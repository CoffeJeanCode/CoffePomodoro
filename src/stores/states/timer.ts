...
export const useTimerState = create<TimerState>()(
	persist(
		(set) => ({
			...initialState,
			setRemainingTime: (time) => set(() => ({ remainingTime: time, remainingTimeText: getTime(time) })),
			setFinishTime: (finishTime) => set(() => ({ finishTime, finishTimeText: getEndTime(finishTime) })),
